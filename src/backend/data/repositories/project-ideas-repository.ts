...
import {
    ...
    repoDetails
} from './interfaces/i-project-ideas-repository'
...
export class ProjectIdeasRepository implements IProjectIdeasRepository {
    ...
    async create(projectIdea: ProjectIdea): Promise<repoDetails> {
        ...
        const {
            ...
            nodeId,
            name: repoName
        } = await this.createRepoREST(projectIdea)

        // Upload idea
        const NEW_BRANCH_NAME = 'review'
        await this.createNewBranch(repoName, NEW_BRANCH_NAME)
        await this.uploadIdea(
            repoName,
            projectIdea.readmeBase64,
            NEW_BRANCH_NAME
        )

        // Add tags (topics) to the repository
        const PROJECT_IDEA_REPO_TAG = 'idea'
        this.addTags(repoName, [
            ...projectIdea.tags,
            PROJECT_IDEA_REPO_TAG
        ]).then(() => console.log('addTags END'))
    ...
    // TODO delete
    private async createRepoGraphQL(
        projectIdea: ProjectIdea
    ): Promise<{ name: string; nodeId: string }> {
        const ghApp = await GitHubApp.instance
